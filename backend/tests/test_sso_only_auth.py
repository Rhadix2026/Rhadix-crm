"""SSO is de enige authenticatieweg (CRM).
...
CRM wordt intern gebruikt maar hangt in dezelfde SSO-keten: Datavalidatie geeft
het token uit, CRM valideert het en provisioneert de gebruiker just-in-time.
"""
import pytest
...
def test_beschermd_crm_endpoint_werkt_met_sso(client, auth):
    """Een echte, beschermde CRM-route werkt met SSO — niet alleen /auth/me."""
...
def test_wachtwoord_wijzigen_geblokkeerd(client, auth):
    r = client.patch("/api/auth/me/password", headers=auth,
                     json={"current_password": "Rhadixcrm26!", "new_password": "NieuwWachtwoord1!"})
    assert r.status_code == 403, r.text
