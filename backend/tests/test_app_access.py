"""Applicatietoegang op basis van de centrale apps-claim (CRM).
...
# Een beschermde route; CRM heeft geen server-to-server routes.
BESCHERMD = "/api/crm/organisaties"
...
def test_8_directe_aanroep_zonder_toewijzing_enforce(client, monkeypatch):
    ...
    for route in (BESCHERMD, "/api/crm/dashboard", "/api/tasks/summary"):
...
def test_9b_beschermde_route_zonder_token_blijft_401(client, monkeypatch):
    """CRM heeft geen publieke API-routes; zonder token blijft het 401, geen 403."""
    monkeypatch.setenv("APP_ACCESS_ENFORCE", "on")
