"""admin.py — PLATFORM_ADMIN: organisaties (tenants) en gebruikers beheren."""
...
def _user_dict(u: User) -> dict:
    return {"id": str(u.id), "email": u.email, "full_name": u.full_name,
            "role": u.role.value, "is_active": u.is_active, "tenant_id": str(u.tenant_id),
...
def create_tenant(body: CreateTenantRequest, db: Session = Depends(get_db), _: User = Depends(_platform)):
    ...
    return {"id": str(tenant.id), "slug": tenant.slug, "name": tenant.name, "admin": _user_dict(admin)}
...
def platform_stats(db: Session = Depends(get_db), _: User = Depends(_platform)):
    return {"tenants": db.query(Tenant).count(),
