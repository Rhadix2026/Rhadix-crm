...
| Datum | Wijziging |
|---|---|
| 2026-07-06 | Krachtenveld-paneel gecorrigeerd: toont nu de **aangesloten zorgaanbieders** van de RSO (organisaties met rso_naam = deze RSO, via de bestaande ?rso-filter) i.p.v. de contactpersonen — dat was de eigenlijke wens. Per aanbieder: naam, type/werkgebied, betrouwbaarheid en aantal contacten. Build groen, staging-deploy groen. |
| 2026-07-06 | Krachtenveld-detail: inline uitklapbaar paneel met de contactpersonen van de gekoppelde organisatie (naam, rol/rolniveau, e-mail/mailto, telefoon), geladen via `listCps(?organisatie_id)`. Backend `_kv` levert nu `organisatie_naam` (via de `organisatie`-relatie) voor de paneelkop. Gekozen boven een losse knop/tab-sprong zodat de gebruiker in de analyse-context blijft. 25 tests groen, build groen. |
| 2026-07-06 | Fix krachtenveld-overzicht (staging): (1) stakeholders met dezelfde invloed+betrokkenheid vielen exact op elkaar → `layoutStakeholders()` waaiert ze uit in een cirkel per matrixcel (namen niet meer over elkaar). (2) Tegel-teller `aantal_stakeholders` bleef verouderd: `saveSh`/`removeSh` verversten alleen het detail (`refresh()`), niet de lijst → `reloadList()` toegevoegd na stakeholder toevoegen/verwijderen en bij terug-naar-overzicht. (Generator dupliceert niet: maakt elke keer een nieuw krachtenveld.) Build groen, naar staging gepusht (a75c716), deploy groen. |
